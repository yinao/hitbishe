...
class HashForest:
    ...
    def setup(self, sample_size, node_size=15, tree_size=5):
    ...
    def build(self, data):
        ...
        for _ in range(self.tree_size):
            ...
            tree = self.build_tree(data[sample_index, :], k, sample_attr_len, numpy.array(weight), 0, 8)
            self.forest.append(tree)

    def build_tree(self, data, k, s_a_l, factor, curh, hlimit):
        rows, cols = data.shape
        if rows <= self.node_size or curh >= hlimit:
            node = Node()
            node.external = True
            node.size = rows
            return node
        ...
        node.size = rows
        node.sub_len = s_a_l
        node.begin = random.randint(0, cols-s_a_l)
        node.weight = factor
        ...
        node.left = self.build_tree(data[left_index, :], k, s_a_l, factor, curh+1, hlimit)
        node.right = self.build_tree(data[right_index, :], k, s_a_l, factor, curh+1, hlimit)
        return node
